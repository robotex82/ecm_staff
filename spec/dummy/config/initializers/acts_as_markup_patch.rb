...
#              end
              instance_variable_set("@#{col}", case send(options[:language_column])
              when /markdown/i
                markup_classes[:markdown].new self.read_attribute(col).to_s, *(options[:markdown_options] || [])
              when /textile/i
                markup_classes[:textile].new self.read_attribute(col).to_s, *(options[:textile_options] || [])
              when /mediawiki/i
                markup_classes[:mediawiki].new self.read_attribute(col).to_s, *(options[:mediawiki_options] || [])
              when /rdoc/i
                markup_classes[:rdoc].new self.read_attribute(col).to_s
              else
                self[col]
              end)
            end
